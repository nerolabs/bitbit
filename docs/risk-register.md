...
| 3 | **Unaudited crypto/consensus fails at scale** (toy proof-of-retrieval, quorum-not-BFT chain, gameable reputation). | Med × High | Independent audit + threat model before any "production" claim; keep honest labeling; bug-bounty; Sybil/eclipse hardening. Build-vs-intention audit (2026-08-02) confirmed the *seams* are clean (mechanism swap, not rewrite) but named three consensus gaps for Gate 4: no equivocation detection/slashing/fork-choice (#100), subjective reputation as consensus input (the partition boundary), and expensive real-crypto that must move off the single core loop (B2). See `docs/design/gate4-m0-mechanism.md`. | **planned**; gaps now issue-tracked (#100) |
