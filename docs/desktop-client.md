...
`bitbit client` is the end-user face of BitBit: a single binary that
...
- `-store ~/.bitbit` — identity keyfile, disk store, link book, learned
...
- **Publish** — drag a file; get back its bitbit link and care link.
...
```sh
./build.sh v0.1     # → dist/bitbit-v0.1-<os>-<arch>
```
...
split all underneath. That was the whole arc: `bitbit add file.txt` on
