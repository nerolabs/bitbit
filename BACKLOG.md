...
> **The spine is elsewhere.** The single source of truth for *what is done and
> what is next* is the GitHub **`V1` milestone** and its issues, narrated in
> [ROADMAP.md](ROADMAP.md). This file is a scratch list of captured ideas and
> task notes that have not (yet) been filed as issues — when one matures, file
> it against the tenet/immutable it serves and drop it here. Interim planning
> markers from the project's **learning phase** (milestones, phases, waves,
> tiers) are **retired**; that history lives in git and `docs/buildlog/`. Where
> notes below still say "*done*," read them as captured context, not a tracker.
...
- **Integration tests before every commit.** The suite runs on every
  ...
  (`go test -race`) and a 45% coverage floor are now in.
  Multi-process end-to-end tests are now in too: the `e2e/`
...
- **The website ships with the code.** *Done.* `ROADMAP.md`
...
### Housekeeping
- Cut the **V1 release** and publish signed binaries once the tenets are
  field-proven multi-machine (see the `V1` milestone and the three-stage
  cadence in [ROADMAP.md](ROADMAP.md) — RC line at `0.9.0`, V1 at `1.0.0`;
  the experimental `0.x` learning tags don't count); the site's downloads
...
- **Column-based placement** — *done.* Placement, provider
  ...
  anti-affinity sims.
- **Failure-domain-aware placement** — *done.* Nodes carry a
...
- **Demand-responsive dispersion** — *done (push half).* Each node
...
- **Repair preserves anti-affinity** — *done.* Repair used to
...
- **Dispersion audit** — *done.* The caretaker sweep now, per
...
distribution) are tracked as issues under the `V1` milestone (see
